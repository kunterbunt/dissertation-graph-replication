...
def plot(filename):
	...
	with open("_data/" + filename + "__full.json") as json_file:
		...
		plt.plot(x, yAcc, label='fully observable: accuracy', color=colors[0], linestyle=':', linewidth=LINE_WIDTH)
		plt.fill_between(x, yAcc_m, yAcc_p, alpha=0.5, color=colors[0], linewidth=LINE_WIDTH)
		# Loss
		...
		plt.plot(x, yLoss, label='fully observable: loss', color=colors[0], linewidth=LINE_WIDTH)
		plt.fill_between(x, yLoss_m, yLoss_p, alpha=0.5, color=colors[0], linewidth=LINE_WIDTH)		
	...
	with open("_data/" + filename + "__partial.json") as json_file:
		...
		yAcc_m = running_mean(yAcc_m, RUNNING_MEAN_AGGREGATE)
		yAcc_p = running_mean(yAcc_p, RUNNING_MEAN_AGGREGATE)
		plt.plot(x, yAcc, label='partially observable: accuracy', color=colors[1], linestyle=':', linewidth=LINE_WIDTH)
		plt.fill_between(x, yAcc_m, yAcc_p, alpha=0.5, color=colors[1], linewidth=LINE_WIDTH)
		# Loss
		yLoss, yLoss_m, yLoss_p = json_data[dict_label_ci_loss]
		yLoss = running_mean(yLoss, RUNNING_MEAN_AGGREGATE)
		yLoss_m = running_mean(yLoss_m, RUNNING_MEAN_AGGREGATE)
		yLoss_p = running_mean(yLoss_p, RUNNING_MEAN_AGGREGATE)
		x = np.array(range(1, len(yLoss) + 1))
		plt.plot(x, yLoss, label='partially observable: loss', color=colors[1], linewidth=LINE_WIDTH)
		plt.fill_between(x, yLoss_m, yLoss_p, alpha=0.5, color=colors[1], linewidth=LINE_WIDTH)		

	fig.set_size_inches((settings.fig_width*1.75, settings.fig_height*1.2), forward=False)
	fig.tight_layout()	
	plt.legend()
	ax = plt.gca()	
	ax.legend(framealpha=0.0, prop={'size': 9}, loc='upper center', bbox_to_anchor=(.45, 1.25), ncol=2, columnspacing=.5)
...
def simulate_partially_observable(filename, n_channels = 8, n_neurons = 128, n_reps = 1, split = 1, max_t = 10000):
	...
	for rep in range(n_reps):
		...
		# Make an initial observation at a randomly selected channel.
...
def simulate_fully_observable(filename, n_channels = 8, n_neurons = 128, n_reps = 1, split = 1, max_t = 10000):
	filename = "_data/" + filename + ".json"
...
if __name__ == '__main__':
	filename = "netsys_comparison_loss_over_time"
	...
	parser.add_argument('--max_t', type=int, help='Number of time slots (default: 50000).', default=50000)
	...
	if not args.no_sim:
		...
		simulate_fully_observable(filename + "__full", args.n_channels, args.n_neurons, args.n_reps, args.split, args.max_t)
		print("Simulation: partially observable")
		simulate_partially_observable(filename + "__partial", args.n_channels, args.n_neurons, args.n_reps, args.split, args.max_t)
	if not args.no_plot:
		plot(filename)
